...
async function levelInfo(dir: string): Promise<{ levelName: string; mcVersion: string | null; lastPlayed: string | null }> {
  try {
    ...
    return { levelName: info.levelName, mcVersion: info.versionName, lastPlayed: (await stat(file)).mtime.toISOString() }
  } catch {
    return { levelName: 'World', mcVersion: null, lastPlayed: null }
  }
}
...
export class WorldStore {
  ...
  async add(source: string, name: string, opts: { signal?: AbortSignal; onProgress?: (d: number, t: number) => void } = {}): Promise<string> {
    ...
    await writeFile(join(this.storeDir, slot, 'pughcraft-world.json'), JSON.stringify({ name, addedAt: new Date().toISOString() }))
    return slot
  }
  ...
  async activate(slot: string): Promise<void> {
    ...
    await rm(join(target, 'pughcraft-world.json'), { force: true })
